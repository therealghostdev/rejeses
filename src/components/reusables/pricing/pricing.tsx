...
export default function Pricing({ item }: PricingProps) {
  ...
  const [pricingData, setPricingData] = useState<PricingData['individuals'] | PricingData['group']>([]);

  const filterCardData_byState = () => {
    if (individuals) {
      setPricingData(item.individuals);
    } else {
      setPricingData(item.group);
    }
  };
  ...
  return (
    <section className="w-full flex flex-col gap-4 px-6">
      ...
      <div className="flex justify-center items-center gap-2 my-4">
        <button
          className={`py-3 px-6 rounded-md -mx-2 ${individuals ? 'bg-[#FFFFFF] text-[#89C13E]' : 'bg-[#DBE1E7] text-[#848484]'}`}
